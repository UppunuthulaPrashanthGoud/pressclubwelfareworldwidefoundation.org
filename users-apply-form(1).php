...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'register_user') {
    ...
    $user_type = 'member';
    $valid_until = ($payment_method === 'online') ? date('Y-m-d', strtotime('+1 year')) : null;
    ...
    // Handle optional press card upload
    if (!empty($_FILES['Press_card']['name']) && $_FILES['Press_card']['error'] === UPLOAD_ERR_OK) {
        $extension = strtolower(pathinfo($_FILES['Press_card']['name'], PATHINFO_EXTENSION));
        $allowed_extensions = ['jpg', 'jpeg', 'png', 'gif', 'pdf'];
        if (!in_array($extension, $allowed_extensions)) {
            logError("Invalid file extension for press_card: $extension");
            echo json_encode(['success' => false, 'message' => "Invalid file type for Press Card. Only JPG, PNG, GIF, and PDF allowed."]);
            exit;
        }
        $uploadResult = uploadFile($_FILES['Press_card'], 'Uploads/profiles');
        if ($uploadResult['success']) {
            $press_card = $uploadResult['filename'];
        } else {
            logError('Press card upload failed: ' . $uploadResult['message']);
            echo json_encode(['success' => false, 'message' => $uploadResult['message']]);
            exit;
        }
    }
    ...
    do {
        $registration_id = 'PCWWF/' . str_pad(mt_rand(1, 99999), 5, '0', STR_PAD_LEFT);
        $stmt = $db->prepare("SELECT COUNT(*) FROM users WHERE registration_id = ?");
    ...
    try {
        $stmt = $db->prepare("
            INSERT INTO users (
                ...
                aadhar_back, press_card, order_id, payment_id, payment_proof, payment_method, 
        ...
        echo json_encode([
            ...
            'data' => [
                'registration_id' => $registration_id,
                'user_id' => $user_id,
                'receipt_url' => $status === 'approved' ? SITE_URL . '/generate_receipt.php?user_id=' . $user_id . '&type=registration' : null
            ]
        ]);
    } catch (PDOException $e) {
    ...
    <div class="card p-3 p-md-5 form-container">
        <form id="user-apply-form" method="post" enctype="multipart/form-data">
            ...
            <div class="row mb-4 text-center">
                ...
                <div class="col-md-3 mb-4">
                    <h6 class="fw-bold">Press Card <span class="text-muted">(Optional)</span></h6>
                    <div class="image-uploader">
                        <input type="file" id="press_card_input" name="Press_card" class="d-none" accept="image/jpeg,image/png,image/gif,application/pdf">
                        <label for="press_card_input" class="uploader-empty">
                            <i class="fa fa-plus fa-2x"></i>
                            <p>Upload Press Card</p>
                        </label>
                        <div class="uploader-preview d-none">
                            <img src="#" alt="Preview">
                            <div class="uploader-controls">
            ...
            <div class="row mb-4 align-items-center offline-payment-proof d-none">
                ...
                <div class="col-md-4">
                    <div class="image-uploader">
                        <input type="file" id="payment_proof" name="payment_proof" class="d-none" accept="image/jpeg,image/png,image/gif,application/pdf">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content text-center">
            ...
            <div class="modal-body">
                ...
                <p id="offlineMessage" class="text-info small d-none">For offline payments, your registration will be confirmed after payment verification.</p>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // State change handler - populate districts
