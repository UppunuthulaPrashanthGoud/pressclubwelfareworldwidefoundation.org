        ...
        switch ($_POST['action']) {
            ...
            case 'edit':
                try {
                    ...
                    if (empty($membership_type) || empty($designation) || empty($designation_hindi)) {
                        throw new Exception('सभी आवश्यक फ़ील्ड भरें।');
                    }
                    ...
                    if (!in_array($membership_type, $valid_memberships)) {
                        throw new Exception('अमान्य सदस्यता प्रकार।');
                    }
                    ...
                    if ($stmt->fetchColumn() > 0) {
                        throw new Exception('यह पद इस सदस्यता प्रकार के लिए पहले से मौजूद है।');
                    }
                    ...
                    if ($_POST['action'] === 'add') {
                        ...
                        $message = 'पद सफलतापूर्वक जोड़ा गया!';
                    } else {
                        $stmt = $db->prepare("
                        ...
                        $message = 'पद जानकारी अपडेट की गई!';
                    }
            ...
            case 'delete':
                try {
                    ...
                    $message = 'पद सफलतापूर्वक हटाया गया!';
                } catch (Exception $e) {
...
function getMembershipTypeName($type) {
    $names = [
        ...
        'gram_panchayat' => 'ग्राम पंचायत स्तर',
        'block' => 'ब्लॉक स्तर',
        'tehsil' => 'तहसील स्तर',
        'district' => 'जिला स्तर',
        'mandal' => 'मंडल स्तर',
...
<div class="admin-content">
    ...
    <div class="main-content">
        <div class="page-header">
            <h1 class="page-title">
                <i class="fas fa-tags me-3"></i> पद प्रबंधन
            </h1>
            <div class="page-actions">
                <?php if ($action === 'list'): ?>
                <a href="?action=add" class="btn btn-primary">
                    <i class="fas fa-plus me-2"></i> नया पद जोड़ें
                </a>
                <?php else: ?>
        ...
        <div class="admin-card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5><i class="fas fa-list me-2"></i> पद सूची</h5>
                <div class="filter-buttons">
                    <a href="?" class="btn btn-sm btn-outline-primary <?php echo empty($status_filter) ? 'active' : ''; ?>">सभी</a>
                    <a href="?status=active" class="btn btn-sm btn-outline-success <?php echo $status_filter === 'active' ? 'active' : ''; ?>">सक्रिय</a>
                    <a href="?status=inactive" class="btn btn-sm btn-outline-danger <?php echo $status_filter === 'inactive' ? 'active' : ''; ?>">निष्क्रिय</a>
                </div>
            </div>
            <div class="card-body">
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge bg-<?php echo $designation['status'] === 'active' ? 'success' : 'danger'; ?>">
                                        <?php echo htmlspecialchars($designation['status'] === 'active' ? 'सक्रिय' : 'निष्क्रिय'); ?>
                                    </span>
                                </td>
                                <td>
                                    <div class="btn-group" role="group">
                                        <a href="?action=edit&id=<?php echo $designation['id']; ?>" class="btn btn-sm btn-outline-primary" title="संपादित करें">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <button type="button" class="btn btn-sm btn-outline-danger" onclick="deleteDesignation(<?php echo $designation['id']; ?>)" title="हटाएं">
        ...
        <div class="admin-card">
            <div class="card-header">
                <h5><i class="fas fa-<?php echo $action === 'add' ? 'plus' : 'edit'; ?> me-2"></i> 
                    <?php echo $action === 'add' ? 'नया पद जोड़ें' : 'पद संपादित करें'; ?>
                </h5>
            </div>
            <div class="card-body">
                <form method="POST">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="designation" class="form-label">पद (अंग्रेजी) *</label>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="designation_hindi" class="form-label">पद (हिंदी) *</label>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="status" name="status" required>
                                    <option value="active" <?php echo ($designation_data['status'] ?? '') === 'active' ? 'selected' : ''; ?>>सक्रिय</option>
                                    <option value="inactive" <?php echo ($designation_data['status'] ?? '') === 'inactive' ? 'selected' : ''; ?>>निष्क्रिय</option>
                                </select>
                            </div>
                        </div>
                    </div>
...
function deleteDesignation(id) {
    if (confirm('क्या आप वाकई इस पद को हटाना चाहते हैं? यह क्रिया पूर्ववत नहीं की जा सकती है।')) {
