...
$validity_filter = isset($_GET['validity']) ? sanitizeInput($_GET['validity']) : '';
...
try {
    ...
    if ($validity_filter === 'renewal') {
        $whereConditions[] = "u.valid_until IS NOT NULL AND u.valid_until != '0000-00-00' AND u.valid_until < CURDATE()";
    } elseif ($validity_filter === 'valid') {
        $whereConditions[] = "(u.valid_until IS NULL OR u.valid_until = '0000-00-00' OR u.valid_until >= CURDATE())";
    }
    ...
    if ($action === 'export') {
        $query = "SELECT u.*, c.name AS created_by_name FROM users u LEFT JOIN users c ON u.created_by = c.id $whereClause ORDER BY u.created_at DESC";
        $stmt = $db->prepare($query);
        $stmt->execute($params);
        $exportUsers = $stmt->fetchAll(PDO::FETCH_ASSOC);

        // Clear any output buffer to ensure clean CSV file
        if (ob_get_length()) {
            ob_clean();
        }
        ...
        // Output UTF-8 BOM for proper rendering of special characters in Excel
        fputs($output, $bom = (chr(0xEF) . chr(0xBB) . chr(0xBF)));

        // Headers
        fputcsv($output, ['ID', 'Registration ID', 'Name', 'S/D/W Type', 'S/D/W Name', 'Mobile', 'Email', 'Gender', 'DOB', 'Profession', 'Blood Group', 'Aadhar', 'State', 'District', 'Address', 'Working Area', 'Pincode', 'Membership Type', 'Designation', 'User Type', 'Status', 'Payment Method', 'Payment ID', 'Valid From', 'Valid Until', 'Created By', 'Created At']);
        ...
        foreach ($exportUsers as $row) {
            fputcsv($output, [
                ...
                $row['working_area'],
                $row['pincode'],
                ucfirst(str_replace('_', ' ', $row['membership_type'])),
                html_entity_decode($row['designation'], ENT_QUOTES | ENT_HTML5, 'UTF-8'),
                $row['user_type'],
                ucfirst($row['status']),
                $row['payment_method'],
                $row['payment_id'],
        ...
        <?php if ($action === 'list'): ?>
            <div class="page-header">
                ...
                <div class="page-actions">
                    <a href="users-management.php?action=export&search=<?php echo urlencode($search); ?>&status=<?php echo urlencode($status_filter); ?>&membership=<?php echo urlencode($membership_filter); ?>&user_type=<?php echo urlencode($user_type_filter); ?>&created_by=<?php echo urlencode($created_by_filter ?? ''); ?>&validity=<?php echo urlencode($validity_filter); ?>" class="btn btn-success me-2">
                    ...
                    <form method="GET" class="row g-3">
                        ...
                        <div class="col-md-3">
                            <select class="form-select" name="validity">
                                ...
                                <option value="valid" <?php echo ($validity_filter === 'valid') ? 'selected' : ''; ?>>Valid</option>
                                <option value="renewal" <?php echo ($validity_filter === 'renewal') ? 'selected' : ''; ?>>Renewal</option>
                            </select>
                        </div>
                        <?php if ($currentUserType === 'admin'): ?>
                        <div class="col-md-3">
                            ...
                            <?php else: ?>
                                <?php foreach ($users as $u): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php 
                                            if (!empty($u['valid_until']) && $u['valid_until'] != '0000-00-00') {
                                                $isExpired = strtotime($u['valid_until']) < strtotime('today');
                                                echo '<span class="' . ($isExpired ? 'text-danger fw-bold' : 'text-success') . '">' . htmlspecialchars(date('d M Y', strtotime($u['valid_until']))) . ($isExpired ? ' <br><small>(Renewal)</small>' : '') . '</span>';
                                            } else {
                                                echo '<span class="text-muted">Lifetime / Not Set</span>';
                                            }
                                            ?>
                                        </td>
                    ...
                    <ul class="pagination justify-content-center">
                        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                            <li class="page-item <?php echo $i === $page ? 'active' : ''; ?>">
                                <a class="page-link" href="?page=<?php echo $i; ?>&search=<?php echo urlencode($search); ?>&status=<?php echo urlencode($status_filter); ?>&membership=<?php echo urlencode($membership_filter); ?>&user_type=<?php echo urlencode($user_type_filter); ?>&created_by=<?php echo urlencode($created_by_filter ?? ''); ?>&validity=<?php echo urlencode($validity_filter); ?>"><?php echo $i; ?></a>
                            </li>
                        <?php endfor; ?>
                    </ul>
                </nav>
