...
require_once '../includes/congratulations_certificate_helpers.php';
...
$pageTitle = 'Congratulations Certificate Management';
$db = getDbConnection();
...
if ($action === 'edit' && $id > 0) {
    ...
    if ($existingCertificate) {
        ...
        $error = 'The selected congratulations-certificate could not be found.';
        $action = 'list';
    }
}
    ...
    if (!verifyCSRF($_POST['csrf_token'] ?? '')) {
        ...
        if ($formAction === 'add' || $formAction === 'edit') {
            ...
            if (empty($error) && $newFileUploaded) {
                if ($_FILES['certificate_file']['error'] !== UPLOAD_ERR_OK) {
                    ...
                    $uploadResult = uploadFile(
                        ...
                        'img/congratulations_certificates',
                    ...
                    if (!$uploadResult['success']) {
                        ...
                        $newType = congratulationsCertificateDetermineType($uploadResult['filename']);
                        ...
                        if (
                            ...
                            $oldFilePath = congratulationsCertificateGetDiskPath($currentCertificate);
            ...
            if (empty($error)) {
                if ($formAction === 'add') {
                    $stmt = $db->prepare("
                        INSERT INTO congratulations_certificates (title, file_path, file_type, created_at, updated_at)
                        VALUES (?, ?, ?, NOW(), NOW())
                    ");
                    $stmt->execute([
                        $formData['title'],
                    ...
                    header("Location: congratulations_certificates.php?success=" . urlencode('E-certificate added successfully.'));
                    exit;
                }

                $stmt = $db->prepare("
                    UPDATE congratulations_certificates
                ...
                header("Location: congratulations_certificates.php?success=" . urlencode('E-certificate updated successfully.'));
                exit;
            }
            ...
            if ($certificate) {
                $stmt = $db->prepare("DELETE FROM congratulations_certificates WHERE id = ?");
                $stmt->execute([$certificateId]);
                ...
                header("Location: congratulations_certificates.php?success=" . urlencode('E-certificate deleted successfully.'));
                exit;
            }
...
$congratulationsCertificates = fetchCongratulationsCertificates($db);
...
<div class="admin-content">
    ...
    <div class="main-content">
        <div class="page-header">
            <h1 class="page-title">
                <i class="fas fa-certificate"></i> Congratulations Certificate Management
            </h1>
            <div class="page-actions">
                <?php if ($action === 'list'): ?>
                    <a href="congratulations_certificates.php?action=add" class="btn btn-primary">
                        <i class="fas fa-plus"></i> Add Congratulations Certificate
                    </a>
                <?php else: ?>
                    <a href="congratulations_certificates.php" class="btn btn-secondary">
        ...
        <?php if ($action === 'add' || $action === 'edit'): ?>
            <div class="admin-card">
                <div class="card-header">
                    ...
                    <?php echo $action === 'edit' ? 'Edit Congratulations Certificate' : 'Add New Congratulations Certificate'; ?>
                </div>
                <div class="card-body">
                    <form method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input
                                ...
                                required
                                placeholder="Enter congratulations-certificate title"
                            >
                        </div>
                        ...
                        <?php if ($action === 'edit' && !empty($formData['file_path'])): ?>
                            <div class="current-certificate-preview mb-4">
                                <div class="preview-header">
                                    ...
                                    <a
                                        href="<?php echo SITE_URL; ?>/congratulations-certificate-view.php?id=<?php echo (int) $formData['id']; ?>"
                                ...
                                <?php if (congratulationsCertificateIsPdf($formData)): ?>
                                ...
                                <?php else: ?>
                                    <img
                                        src="<?php echo htmlspecialchars(congratulationsCertificateGetUrl($formData), ENT_QUOTES, 'UTF-8'); ?>"
                        ...
                        <div class="mt-4">
                            <button type="submit" class="btn btn-primary">
                                ...
                                <?php echo $action === 'edit' ? 'Update Congratulations Certificate' : 'Save Congratulations Certificate'; ?>
                            </button>
                            <a href="congratulations_certificates.php" class="btn btn-secondary">
        ...
        <?php else: ?>
            <div class="admin-card">
                <div class="card-header">
                    <i class="fas fa-list"></i> Congratulations Certificate List
                </div>
                <div class="card-body">
                    <?php if (!empty($congratulationsCertificates)): ?>
                        <div class="table-responsive">
                            <table class="table table-striped table-hover align-middle">
                                ...
                                <tbody>
                                    <?php foreach ($congratulationsCertificates as $certificate): ?>
                                        <tr>
                                            <td style="width: 120px;">
                                                ...
                                                <?php else: ?>
                                                    <img
                                                        ...
                                                        class="img-thumbnail tablcongratulations-certificate-thumb"
                                                    >
                                                <?php endif; ?>
                                            </td>
                                            ...
                                            <td>
                                                <div class="btn-group" role="group">
                                                    <a
                                                        ...
                                                        target="_blank"
                                                        rel="noopener"
                                                        title="<?php echo congratulationsCertificateIsPdf($certificate) ? 'Download PDF' : 'Preview Image'; ?>"
                                                    >
                                                        <i class="fas fa-eye"></i>
                                                    </a>
                                                    <a
                                                        href="congratulations_certificates.php?action=edit&id=<?php echo (int) $certificate['id']; ?>"
                                                        class="btn btn-sm btn-warning"
                                                        title="Edit"
                                                    >
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <form method="post" class="d-inline" onsubmit="return confirm('Delete this congratulations-certificate?');">
                    ...
                    <?php else: ?>
                        <div class="alert alert-info mb-0">
                            <i class="fas fa-info-circle"></i> No congratulations-certificates have been added yet.
                        </div>
                    <?php endif; ?>
                </div>
            </div>
...
.tablcongratulations-certificate-thumb {
    width: 90px;
    height: 90px;
    object-fit: cover;
