...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'get_user_data') {
    ...
    try {
        ...
        if (empty($registration_id) || !preg_match('/^(NDF[0-9A-Z]+|NDFM[0-9]+|NDFR[0-9]+)$/', $registration_id)) {
            logError('Invalid registration ID: ' . $registration_id);
            echo json_encode(['success' => false, 'message' => 'Invalid registration ID format (e.g., NDF003, NDFM001, NDFR001)']);
            exit;
        }
...
<main class="container my-5">
    <h3 class="section-heading text-center"><span>Universal ID Card Download</span></h3>
                    ...
                    <form id="download-form">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="registration-id" name="registration_id" 
                                   placeholder="Enter your registration ID (e.g., NDF003, NDFM001)" required
                                   pattern="(NDF[0-9A-Z]+|NDFM[0-9]+|NDFR[0-9]+)" value="NDFM003">
                            <div class="form-text">Format: NDF003, NDFM001, or NDFR001</div>
                        </div>
...
async function generateUniversalIdCard() {
    ...
    if (!registrationId || !/^(NDF[0-9A-Z]+|NDFM[0-9]+|NDFR[0-9]+)$/.test(registrationId)) {
        Swal.fire('Error', 'Please enter a valid registration ID (e.g., NDF003, NDFM001, NDFR001)', 'error');
        return;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Universal ID Card Download page loaded');
