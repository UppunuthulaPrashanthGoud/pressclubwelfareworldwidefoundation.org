...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($password)) {
        ...
        try {
            $stmt = $db->prepare("SELECT * FROM users WHERE email = ? AND user_type = 'coordinator' AND status = 'approved'");
            ...
            if ($user && password_verify($password, $user['password'])) {
                ...
                $error = 'Invalid coordinator credentials or account not approved.';
            }
        } catch (PDOException $e) {
            $error = 'Login failed. Please try again later.';
            logError('Coordinator login error: ' . $e->getMessage());
        }
    }
}
                ...
                <div class="card login-card shadow-lg">
                    <div class="card-body p-5">
                        <div class="text-center mb-4">
                            ...
                            <h3 class="card-title mt-3 mb-2">Coordinator Login</h3>
                            <p class="text-muted">Access Coordinator Dashboard</p>
                        </div>
                        ...
                        <form method="POST">
                            <div class="mb-3">
                                <label for="email" class="form-label fw-bold">
                                    <i class="fas fa-envelope me-2"></i>Coordinator Email
                                </label>
                            ...
                            <button type="submit" class="btn btn-submit-grad w-100 btn-lg">
                                <i class="fas fa-sign-in-alt me-2"></i>Login as Coordinator
                            </button>
                        </form>
