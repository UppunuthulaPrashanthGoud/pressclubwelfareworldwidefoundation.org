...
// We fetch these before handling the POST so they are available if the page reloads on error
try {
    $awards_stmt = $db->query("SELECT award_name FROM awards_list WHERE status = 'active' ORDER BY award_name ASC");
    $db_awards = $awards_stmt->fetchAll(PDO::FETCH_ASSOC);

    $venues_stmt = $db->query("SELECT venue_name FROM venues_list WHERE status = 'active' ORDER BY venue_name ASC");
    $db_venues = $venues_stmt->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'register_user') {
    ...
    $required_fields = ['name', 'contact_no', 'category', 'award', 'email', 'state', 'city', 'pincode'];
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select name="award" id="award" class="form-control" required>
                            ...
                            <?php foreach($db_awards as $row): ?>
                                <option value="<?php echo htmlspecialchars($row['award_name']); ?>">
                                    <?php echo htmlspecialchars($row['award_name']); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select name="venue" id="venue" class="form-control" required>
                            ...
                            <?php foreach($db_venues as $row): ?>
                                <option value="<?php echo htmlspecialchars($row['venue_name']); ?>">
